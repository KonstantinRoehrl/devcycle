...
| `event` | `gate-fail` / `gate-pass-clean` once per green-gate run, at step 6; `user-correction-at-gate` at any AskUserQuestion the user answers via "Other" — the stage only, never the typed text. Appended only where a run record exists, which means inside a cycle run: `commands/cycle.md` mints the only one, so the gates of a standalone command (`/devcycle:doctor`, `/devcycle:learn`, `/devcycle:onboard`, `/devcycle:review`, `/devcycle:verify`) have no run to append to and sit outside the journal's coverage — a run with no `user-correction-at-gate` events therefore says nothing about them | `playbooks/executing-waves.md`, and every in-cycle surface that asks |
