...
`git add -N` above, which only makes untracked files diff-visible and reverts nothing.) `hooks/block-destructive-git.mjs` structurally enforces this. Before recording a red test as
