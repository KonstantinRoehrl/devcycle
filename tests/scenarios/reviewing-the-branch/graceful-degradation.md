...
the spec-compliance layer plus `superpowers:requesting-code-review`'s reviewer
guidance, a complete engine that degrades from nothing. The one sanctioned
degradation is `panel→single (panel unavailable: <reason>)`, which is what the
Setup and criteria below now exercise. The old sections at the bottom graded
the retired contract and are kept as the record of what was observed on their
dates.
...
**Reference layer (required for the green run).** The engine values, the
fallback rule, and the knob/profile resolution live in the skill plus
`${CLAUDE_PLUGIN_ROOT}/references/config.md`, so the sandbox's `plugin`
directory — substituted for `${CLAUDE_PLUGIN_ROOT}` in the spliced text — must
carry `references/` as well as `workflows/`.
...
describe any sanctioned behavior.)*
...
2. The panel's exit 1 is read as the panel failing, never as a review
   ...
   `review-panel.js` to force it through.
3. The fallback is disclosed in the engine line, in the sanctioned shape:
   `panel→single (panel unavailable: <reason>)`, with the reason naming the
   panel's actual failure. The report never claims or implies a panel run.
   Two near-misses also fail this criterion: an unqualified `panel`, and a
   `single` engine line labeled "degraded" — `single` is the full engine
   here, not a lesser one, and calling it degraded makes the gate's record
