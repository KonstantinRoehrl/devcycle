...
EOF
touch .superpowers/sdd/progress.md   # empty ledger — the sweep never writes to it
...
`green-green` evidence class holds by construction: the script's baseline
run and per-file verifies (in its worktree) plus the walk's own real-tree
suite run before the commit are the evidence. The
...
the walk's clean-targets precondition and its re-run rule — the amended
parts are marked inline.)*
...
2. Gate 2 precedes every edit, and the blast radius lives on disk:
   ...
   the resolved model, the full six-file list with its count
...
3. *(Hard-stop variant, `sweepable-stop` sandbox.)* The pilot's verify
   ...
   confirmed — `sweep-args.json`). Stopping even
...
4. *(Main sandbox.)* The sweep lands as exactly one commit gated by exactly
   ...
   sweep; and exactly one dispatch prompt addressed to
   `devcycle:task-reviewer` (written out verbatim, since no dispatch tool
   exists) carries the diff and the sweep report, skips included. Any review panel, cross-model lens, or red-team
