...
6. Turn 2 ends with a devcycle handoff block (`Stage completed: scoping`) naming `superpowers:brainstorming` as the next stage; still no implementation, and the ONLY files created or modified in the sandbox are the stage's own artifacts — `.devcycle/scope.md` and `.devcycle/state.md` (verify with `git status --short`).
7. The scope summary is written to `.devcycle/scope.md` as the structured goal (end state, in/out of scope, affected areas, constraints, open `<tbd>` items — criterion 5's `<tbd>` appears in the file, not only in conversation); `.devcycle/state.md` is updated before the handoff block with `stage: brainstorm` (the stage to resume at) and `scope: .devcycle/scope.md`; and the handoff block's Artifacts line is `.devcycle/scope.md`.
...
Criterion 6 updated and criterion 7 added 2026-07-23 after the review-fixes bundle made `.devcycle/scope.md` the stage's required artifact (the old handoff allowed `Artifacts: none (scope summary above)`) and added the pre-handoff state update (`stage: brainstorm`, `scope: .devcycle/scope.md`). Note criterion 6 inverted its old "no files created" check: the stage now MUST write its two artifacts, and only those. Both runs: fresh headless subagents (`claude -p`, model `claude-sonnet-5`), isolated config (fresh CLAUDE_CONFIG_DIR holding only auth; init events confirmed `plugins: []`), taskly sandbox rebuilt per Setup in session-temp directories. Red = committed text (`git show HEAD:skills/scoping-interview/SKILL.md`); green = working tree.

- Baseline (red): criterion 7 FAIL — the scope summary lived only in conversation: no `.devcycle/scope.md` anywhere, the handoff's Artifacts line read `.devcycle/state.md`, and the state file (written by the committed stage-entry backstop) was a free-form 6-line variant (`Stage: scoping`, no scope/configured lines) never updated to `stage: brainstorm`. Criteria 1–6 otherwise held (one batch of 4, summary in slot 1, hard stop, no follow-up round, explicit `<tbd>` firing-mechanism marker, handoff naming `superpowers:brainstorming`, only `.devcycle/` writes). Net: RED on criterion 7.
- Result (green): PASSED all seven criteria. One batch of 4 with slot 1 = summary confirmation ("Confirm my read…") — minor accepted variance, options embedded in each question's prose with an explicit free-form escape (plain-message fallback); Turn 1 hard-stops with zero writes; zero follow-up rounds; `<tbd>` explicit. Turn 2 wrote `.devcycle/scope.md` as the structured goal (End state / In scope / Out of scope / Affected areas / Constraints / `## Open <tbd>` carrying the firing-mechanism item), updated `.devcycle/state.md` to `stage: brainstorm` + `scope: .devcycle/scope.md` before the block (minor variance: the state file omits the `# devcycle state` heading and ledger/updated lines), and emitted the handoff with `Artifacts: .devcycle/scope.md` plus "Handing off to `superpowers:brainstorming` next, with this scope as its starting context." `git status --short` shows only `.devcycle/` writes. Net: GREEN.
